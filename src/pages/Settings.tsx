...
import { BonusSlab } from "@/types/bonus";
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs";
import { ChangePasswordPanel } from "@/components/ChangePasswordPanel";

const SettingsPage = () => {
  const { settings, updateSettings } = useClientData();
  ...
  useEffect(() => {
    if (settings) {
      setExpenseRate(settings.companyExpensePercentage);
      setSlabs(settings.bonusSlabs);
    }
  }, [settings]);
  ...
  const handleSave = async () => {
    ...
    try {
      await updateSettings({
        ...settings,
        companyExpensePercentage: expenseRate,
        bonusSlabs: slabs,
      });
